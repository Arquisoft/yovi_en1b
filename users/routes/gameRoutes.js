...
module.exports = (repository) => {
    ...
    router.post('/play', async (req, res) => {
        ...
        try {
            // TODO: confirm exact endpoint with Bilal & Adrian
            const gameyResponse = await fetch(`${GAMEY_URL}/play`, {
                method: 'POST',
            ...
            res.status(503).json({ error: 'Gamey service unreachable' });
        }
    });
    ...
    // Submit a move — only needs coordinates, player inferred from current_turn
    ...
    router.get('/:id/moves', authMiddleware, async (req, res) => {
        try {
            const game = await repository.findGameById(req.params.id);
            if (!game) return res.status(404).json({ error: 'Game not found' });
