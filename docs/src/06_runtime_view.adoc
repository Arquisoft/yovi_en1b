...
[plantuml,"player-reconnection-sequence",png]
----
@startuml
actor Player1
participant "React Frontend" as FE
...
note over Player1,WS: Connection Lost
WS -> WS: Detect disconnection
WS -> Player2: Notify: opponent disconnected
Player2 <- FE: Display "Waiting for opponent..."

note over Player1: Player reopens application
Player1 -> FE: Login
FE -> API: GET /api/games/active
API -> DB: Query active games for user
DB --> API: Return active games list
API --> FE: Active games
FE --> Player1: Show "Resume Game" option
Player1 -> FE: Click "Resume Game"
FE -> WS: Reconnect to game room (gameId)
WS -> API: Request current game state
API -> DB: Retrieve game state
DB --> API: Current board state
API --> WS: Full game state
WS --> FE: Board state + turn info
FE --> Player1: Restore game UI
WS -> Player2: Notify: opponent reconnected
Player2 <- FE: Update status "Opponent back"
note over Player1,Player2: Game resumes
@enduml
----

*Notable aspects:*

* Game state persists in the database even if players disconnect
* Players can rejoin games in progress
* The system distinguishes between temporary disconnections and game abandonment
* After a timeout period (e.g., 1 minutes), the game may be forfeited
* WebSocket connection is re-established seamlessly
* The complete hexagonal board state is preserved and restored
* Opponent receives real-time notifications about connection status
