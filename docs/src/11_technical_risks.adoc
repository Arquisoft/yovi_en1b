...
|*1 — Resolved*
|*Legacy bot strategy keys in MongoDB* (Fixed)
Early versions stored bot results under `easy`, `medium`, `hard` in `statistics.vs_bot`. Current code uses `random`, `defensive`, `ai`, and `mcts`. The schema migration has been applied and the database is now consistent.
...
|*2 — Resolved*
|*Section 8 (Cross-cutting Concepts) not documented* (Fixed)
The arc42 cross-cutting concepts section is now fully documented, covering the domain model, YEN notation, win detection algorithm, security concepts, inter-service communication, persistence model, bot AI strategy pattern, game variants, and error handling.
|Resolved.
...
|*5 — Resolved*
|*arc42 sections 9, 10, 11, 12 incomplete* (Fixed)
Architecture Decision Records, quality scenarios, risks, and the glossary have been completed. ADRs covering MongoDB, Azure, and JWT are documented in the project wiki.
|Resolved.
|===